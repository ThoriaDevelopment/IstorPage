...
one drifts. That is not hypothetical: the old site's mark exporter grew exactly
such a list before this file existed.

Carried forward from the old site unchanged, because it is machine plumbing
rather than site content. It says where the tools are, not what to build with
them, so the redesign does not invalidate it. The original is kept at
`OldVersion/tools/bench.py`. Generators that use it belong in this same
directory, so that `import bench` keeps working.
