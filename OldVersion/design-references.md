...
Istor" is the translation. Last updated 2026-09-17.
...
- Not adopted from this reference: the alternating showcase rows with product
  visuals, which the page flow above describes. That structure stayed rejected.
- Adopted later, and different in kind: one showcase scene on the homepage,
  Thoria's commit `c89b126` on 2026-09-01. A single sheet carrying a source
  note and a citation turns over to show the answer it came from, with two
  citation chips beside it, under the heading "One quiet room for everything."
  One scene and one object, not a repeated row per feature. It is recorded in
  `.claude/skills/istor-brand/SKILL.md` under structure. The note that used to
  stand here, that Thoria had decided against showcase visuals entirely, was
  written on 2026-09-01 and was overtaken by that commit the same day.
