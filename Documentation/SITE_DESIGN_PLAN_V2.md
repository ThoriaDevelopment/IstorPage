...
**And the fold made two searches that had agreed stop agreeing, which is the kind of bug the plan is
supposed to catch.** The directory's own find field filters the list in front of the reader and this
dialog searches the other seventy-four pages; they were supposed to share one word rule - and, it
turned out, one body of text too, which is the correction immediately below - but the fold had quietly
given the dialog a new rule, so the two answered one query two ways. Measured on the directory's own
corpus: typing `hallucinations` **hid the page called "What is AI hallucination?"** (0 of 75 entries
shown) while the same word found it in the dialog, `gpus` showed nothing where the dialog found five
pages, and `models` showed **16 entries against the dialog's 42**. So the rule now has one
implementation in `/search.js`, published as `istorMatch` for the generated find script to call - read
at each keystroke rather than at load, because that script is inline and this file is deferred - and the
field keeps its own substring test as the fallback, so it cannot stop filtering because a script it does
not own went missing.

**The same correction had to be made once more, one level down, because a shared rule about words is
not a shared answer.** The field filtered each entry on the entry's own title and summary; the dialog
reads those plus every section's heading and its first line. So the two now agreed about *spelling*
and still disagreed about *content*: `parameters` returned **1 entry against 9 pages**, `models` **42
against 58**, `tokens` 3 against 8, `privacy` 2 against 6. The fetch and the parse are now one memoized
`index()` that `/search.js` publishes as `istorIndex`, and the field reads each page's own text out of
it - lazily, on the first keystroke anybody types, which is most page loads never. Until it arrives,
and if it never does, the entry's title and summary are what is matched, so the list filters on the
keystroke rather than after a round trip. The drift this creates next is that the page could fetch and
parse its own copy of the corpus, so the gate asserts the page does **not** name the index file at all,
only the accessor. And the claim that proves the whole arrangement is the one a reader would make
themselves: the same query, asked in both controls on one page, must produce the same count - measured
on `parameters`, where the field now says **9 of 75 pages match** and the dialog says **the first 8 of
9** while showing eight of them. Two facts hold the pair together and neither is visible from one file: the page
calls the name the script publishes, and the page carries **no second copy of the fold table**. Both are
asserted by `verify-links.py`, and both were shown failing before they were trusted; the behaviour is asserted by the audit,
including the fallback claim, which reads as the size of the old disagreement: with the shared matcher
deleted, `models` shows **16 entries** where the fold shows **42**.
...
| `library.search_bytes_ceiling` **(new row, 2026-09-21)** | **84,000 B** over the **67,608 B** search index, the generator's own ceiling stated a second time here: the palette fetches that file whole, lazily, the first time it opens. `library.files` gains `/search.js` at **25,236 B** exact, and `/styles.css` moves 59,140 → **67,945 B** for the trigger's rule and the dialog's own type, grounds and measured ratios. `/search.js` moved three times on 2026-09-21, and each move is visible here as the one-line change it is: 14,371 → **14,732** for a null guard `audit-palette.py`'s self-test found by making the fetch early, then → **25,236** for the fold, for moving it above the palette, and for the accessor the directory's field reads it through, which is mostly its own reasoning written down (why each family exists, what it was measured to buy, why two directions that look useful are absent, and why the rule and the corpus sit above the palette's early return) |
| `verify-links.py` — the search palette **(new pass)** | **14 assertions** (93 → **108**), and the coupling is the point: every carried page, the directory **and the landing** carries the trigger and the script (a page that lost either half looks exactly like a page that has them), the script's `INDEX` names a file this build writes, that file is the generator's output **byte for byte**, the record set equals the **artifact's** published pages rather than the generator's own input, the classes the script builds have rules in the stylesheet the pages load, and the mark's colour is **recomputed from the tokens** (`--ink` under `--cite-wash` 15.15:1 and 14.60:1 light, 13.15:1 and 11.68:1 dark, against 4.53:1 and 4.36:1 for the accent it replaced), so the note explaining that choice fails the build rather than going quietly stale. The pass also runs its two tools' own proofs, `make-search-index.py --self-test` (**11 doctored files**) and `add-search-trigger.py --check` |
...
| `verify-links.py` — the temperature page's slider **(new pass, 2026-09-21)** | eight assertions (108 → **116**), and the coupling is the point: the control is carried by exactly one page and named by exactly one script tag (a copy pasted onto another page would be a bar with no plate above it, and a page that lost the script ships a slider that cannot move); the table the script reads is **recomputed from `odds.py`**, the module the plate above it is drawn from, because the script does no arithmetic on purpose; the bar a reader meets before any script runs is the table's row for the starting setting, its widths sum to the free width the bar declares; the slider's range covers every setting the plate draws, steps by the table's own step and starts on the drawn one; the control is authored `hidden` so the script is what puts it on the page; its sentence names the two ends of the bar; and the stylesheet draws every class the markup uses while the print block leaves the instrument off the paper. What is not claimed: that the script moves the bar — behaviour is measured in a browser, by the dial probe, not read out of a file. Shown failing five ways before it was trusted (script tag removed, a table width changed, the control made visible in the source, the range shortened past the plate's high setting, a class renamed out of the stylesheet) |
| `verify-links.py` — the close's rim **(new pass, 2026-09-21)** | three assertions (116 → **119**), and the failure it exists for is the worst kind a CSS coupling can fail: the crown's tick ring passes at the wordmark's mid-height in full primary ink, the type brings the separation instead — a `::before` halo stroked in the field's base colour — and the halo is **spelled by `content: attr(data-mark)`**, so renaming the attribute on either side resolves `attr()` to nothing, deletes the rim silently, and leaves every other gate green. The clauses: `data-mark` is the string the mark spells (compared against the paragraph's own text, because a halo spelling the wrong word is worse than none); the layer reads that attribute by name, carries a stroke, and sits **behind** the fill (a rim painted over the letterforms is the defect it fixes); the print world remaps the stroke to paper, because the rim is the field's ground by construction and the print world strips that ground. The halo declares its content twice — direct `attr()` first, then `attr(data-mark) / ""` — so a renderer that understands the alt-text form keeps the wordmark out of the accessibility tree once, and one that does not keeps the rim. Shown failing five ways (attribute renamed on the page, `attr()` renamed in the stylesheet, halo moved over the fill, print remap dropped, stroke removed) |
| `audit-palette.py` **(new tool, 2026-09-21)** | the third tool that drives a page rather than reading one, and it exists because the palette's claims are all about *behaviour*: it clicks the trigger and asserts the link was **prevented** (the page is still the page), that `role`/`aria-expanded` are added **only once a script is there to honour them**, and that the trigger is a plain `href="/library/"` on disk. **30 claims** across five surfaces: a carried page (no dialog in the DOM until asked for, **zero** requests for the index before the click and exactly one after, rows with their matched words marked, ArrowDown entering and leaving the result list at its ends, Enter navigating to `/what-is-quantization/`, focus returning to the trigger on close, what was typed surviving a reopen, and no `img`/`script`/`iframe` ever reaching the panel's text), the directory (`` ` `` still belongs to the field in front of the reader, and **Cmd/Ctrl+K** opens the palette over it), the landing (the close's trigger opens the same dialog and a result links into the library), reduced motion (opens, searches, `opacity 1`), and **the fold** - the seven claims that say what the palette counts as a match, typed as queries: `hallucinations` **0 → 3** pages, `llms` **0 → 2**, `gpus` **0 → 8**, `quantisation` **1 → 6**, `tokeniser` **0 → 1**, `boundary` **0 → 1**, while `cars` still finds nothing although `car` is inside 22 indexed pages and `mixture of experts` finds nothing at all. Its `--self-test` doctors **20 files** (19 patched scripts and pages plus a doctored index) and requires each to fail the claim it breaks: five remove one published row of the fold or one guard around it, and two doctor the **generated directory** rather than the script that generates it, so what is doctored is what a reader is served. Three of the directory claims are the same question asked of the other search - that field finds `hallucinations` (2 entries of 75 where the plain rule showed none), refuses `cars` like the palette does, and still filters when `/search.js` never loads, at which point `models` shows **16 entries against the fold's 42**, which is the size of the disagreement in one number. **Two real defects were its catches**, and neither could have been made from reading a file. The first is in the **directory**: the index lifts a real page's header, and the header carries the trigger, so a build that lifted the theme script but not the palette's shipped a directory whose control navigated to the page it was already on, with a Cmd+K that did nothing - a dead control that looked exactly like a working link to every static check in the repository, because a lifted header carries no list of what it depends on. `verify-links.py` now demands both halves (`data-search-open` **and** `/search.js`) in the generated directory, and the generator lifts the script tag by name and raises if it is absent. The second is in the **fold itself**, found the first time its claims ran: the row that completes `boundary` into `boundaries` appended instead of replacing, so the palette was searching for `boundaryies`, a word no page in the library has ever written - and the reason it was caught at all is that the byte ledger made a change to `/search.js` something to look at. A third, smaller one came from measuring rather than reasoning: the strip row refused a word ending in `us`, on the theory that `status` would fold to `statu` - it does, harmlessly - and that guard was costing the plural of the library's own acronym, so `gpus` found nothing while `gpu` is a word of eight pages |
...
| `make-token-rows.py` **(new plate, 2026-09-21)** | the seventh carried page to carry a plate, and it is the one whose claim is a shape: `/what-is-tokenization/` argues in prose that tokens sit between characters and words, so the page now shows the argument. The same sentence cut three ways, **35 pieces by letter, 9 by token, 6 by word**, with each count drawn beside its own row; the row a model reads is the only row in ink and the only one where nothing is unknown, so its one foreign name is marked twice in **the same four letters** - a dashed box in the word row, a bracket under the token row. Two variants like every plate here, swapped by the page's own rule at 560px. **The self-test is where the work went.** Every string is placed by `plan()` before any of it is drawn, so "does a label sit inside the plate" and "does a label sit on another" are questions about data rather than arithmetic buried inside text calls, and the first version was failing both of them: a title running off the narrow plate, and the bracket's label sitting on the row below. **Two defects were found by looking at it rather than by a claim**, both after it passed every check it had: the bracket was a plain horizontal rule five units under the band, which on a page of links reads as an underline, and it began one character cell early, under the gap before the name instead of under the name. Both are fixed and both are now claims - the bracket is compared against the span the two pieces actually occupy **and** against the span the word row's box occupies, because the two rows marking the same four letters is this plate's argument written as arithmetic. Its `--self-test` draws into a **temporary directory**, and that is a lesson rather than a tidy-up: the probes for this plate doctor it, one of those doctored probes ran the drawing's own writer, and the doctored plate it wrote was still in `Source/figures/` for the build to serve. `verify-figures.py` reported it stale exactly as it should. The writer now takes its directory as an argument and the self-test compares the shipped files' bytes before and after it runs, so proving the plate can fail cannot ship the plate that failed. **44 checks** (38 → 41 for this plate, → 44 for the temperature one below); it is registered in the library's include table, so a carried page can carry a figure at all |
| `make-temperature.py` **(new plate, 2026-09-21)** | **the first figure here that computes.** `/what-is-temperature/` says "Raised, the odds flatten, and less likely candidates start winning slots", and that sentence is a picture, so the page draws it: four next-word candidates, **the same four under three settings**, each share a segment of a bar with the top share printed at the end of its row (**57.4% → 41.3% → 32.9%**, while the last candidate climbs **5.2% → 18.1%**). The segments are `var(--ink)` at four `fill-opacity` steps, and the faintest step is measured against **both** grounds, because a filled mark that carries meaning needs 3:1 and half the readers are in the dark theme. **Its claims are the arithmetic**, which is what makes it checkable rather than decorative: the check recomputes softmax from `LOGITS` and `TEMPERATURES` and compares it to the drawn widths segment by segment, reads each printed percentage back off the segment it labels, and asserts the flattening in **both** directions against the **drawn widths** rather than against the odds, because widths are what a reader reads - the top segment has to lose a quarter of its length and the last has to more than double, on the narrow plate, which is the harder of the two. It carries **six internal doctors** and requires each to be caught by the claim it breaks: a segment five units wider than its share, a percentage a hair off its own segment, a share drawn as a sliver, a ramp step made invisible, a swatch that is not its segment's fill, and a percentage that belongs to another candidate. The candidate list and the gaps between the odds are **made up**, and the plate says so in a note. The reshaping is not |
| `make-agent-loop.py` **(new plate, 2026-09-21)** | the thirteenth, and the **first flowchart** here. `/what-is-an-ai-agent/` argues in paths - "ordinary chat is one pass of inference: question in, answer out, done. An agent inserts a middle" - and its heading is the drawing: **a loop, not a single answer**. The plate draws two paths that share their start and end boxes: chat's straight pass in mist, the agent's bend through a tool and back into the model in the witness colour, and the branch itself - "call a tool, or decide it is done" - drawn as what the page says it is, the model's exactly two ways out. **Its checks are topological rather than numerical**, the first of their kind in the repository: every arrow tip lands on a node's boundary; the tool is touched exactly once in and once out; the model is touched four times, which is what makes it a revisited box rather than a sequence; chat's pass touches no node between its endpoints; and the drawn shafts and heads are required to be the segment functions' own outputs, because the shaft is drawn short of its tip and a check comparing the drawn endpoints against the logical ones fails on the geometry that makes an arrow an arrow. The touch counts are counted on **both** endpoints - a first version counted tips only and called a loop with four contacts "two". Its seven doctors each fail the claim they break. **Figures 57 → 60** |
| `make-rerank-flow.py` **(new plate, 2026-09-21)** | the twelfth, and the first **bump chart** in the repository. `/what-is-reranking/` argues in counts and orders and shows neither: "retrieval pulls maybe twenty candidates"; "hands the top handful to the model"; and its sharpest sentence is about the reordering itself - a reranker catches "the near-misses where topics sound alike but the answer is absent" and "the matches where wording differs but meaning aligns". The plate is that sentence as one drawing: the **same twenty candidates** as two columns, first pass's order and reranker's order, one bracket per column spanning exactly the budget's four, and the two candidates the sentence is about followed in the witness colour - the near-miss demoted out of the bracket, the paraphrased answer promoted into it, their two lines crossing where the swap happens, because a swap nobody can see argues for nothing. **The probe earned its keep on this one**: the first version's `check_bar_style` collected findings and returned a literal `[]`, so every style defect passed in silence, and the fill-set it should have been enforcing was itself wrong (text fills with `--ink`); both are fixed, and a drawing-independent count of the two witness lines was added because a doctored drawing that stopped drawing them could otherwise pass a stroke-set check that parsed through the same broken pipeline. Its six doctors each fail the claim they break - the demotion, the promotion, a third witness, a bracket past the budget, an endpoint off its row centre, a hex colour. **Figures 54 → 57** |
| `make-vram-split.py` **(new plate, 2026-09-21)** | the thirteenth, and the first **memory-state** plate. `/what-is-vram/` argues in arithmetic it never draws: quantization and VRAM are "two halves of one decision", the split puts "the layers that fit" on the GPU and "the rest run on the CPU", and the weights are "the biggest tenant, not the only one". The plate is one model against one card, twice, at **one scale** - a gigabyte is the same width in every bar, so nothing is resized to look better. At 8 bits, 16 of 32 layers fit and the count label sits **in the empty remainder of the RAM bar**, the split's own words in the split's own space; at 4 bits, all 32 fit and the leftover ground is the tenancy headroom, drawn. The checks read the drawn segments, not the intent: every layer accounted for once, the card's bar identical in both states, an 8-bit layer exactly 2.5x a 4-bit one (the bit-width as a ratio is the quantization claim in pixels), the 4-bit fill under nine tenths, and each panel's label made to name the bit-width its arithmetic drew. Found by looking, not by checks: the first composition put the RAM count label **on the spilled segments** - label-vs-label was checked, label-vs-drawing was not, so the model-extent check was added. Its six doctors each fail the claim they break. **Figures 57 -> 63** |
| `make-reading-bar.py` **(new plate, 2026-09-21)** | the eleventh, and the page it draws for is the one that names its own instrument three times without showing it: `/what-is-tokens-per-second/` says a reader needs "five to ten", that below ten "long answers start to drag", below five "the tool feels broken", that twenty or thirty "is streaming faster than you can read", and then argues entirely against **"the reading bar"** it never draws. The plate draws the bar and the page's whole first section falls out of it: four example speeds on one axis (4 under, 12 clearing, 25 faster than reading, 90 a datacenter number the eye cannot spend) with the bar a vertical witness line at six, and a second panel where x is answer progress and y is the rate, so the drift "as an answer grows" is a line sagging from 30 to 18 that must finish **above** the horizontal bar, because that is what "usable" means here. One threshold, read on each panel's own axes. The checks read the drawing back rather than the constants that drew it - the four numbers' drawn x positions through the scale the marks use, the path's own points for the sag and the finish above the bar, the two witness lines' spans - and a label-vs-path clearance check exists because the first version's start label grazed the line on the tall variant and every label-only overlap check passed it: the path is not in `boxes()`, so it is checked segment by segment against every label rectangle. Placement and check share one interpolation helper (`deepest_over`), the same discipline that keeps the printed percentages the bars' own. Shown failing six ways (a speed drawn off its own scale, the drift finishing below the bar, the drift rising mid-answer, the vertical bar off its value, the horizontal bar not spanning its panel, a hex colour smuggled in). **Figures 51 → 54** |
...
**A plate is now something a carried page can carry, and nine pages have one.** Thirty-two drawings
sit in `Source/figures/`; five plates are inlined into `Source/index.html`, and **eight carried pages**
carry a pair of their own (`what-is-a-context-window`, `what-is-a-gguf`, `what-is-quantization`,
`what-does-q4-k-m-mean`, `how-much-ram-for-local-ai`, `why-ai-hallucinates-citations`, and the two that
joined them on 2026-09-21: `what-is-tokenization` and `what-is-temperature`). The mechanism is the second include table: `LIBRARY_INCLUDES`
...
`Source/tools/audit-palette.py` asserts the thirty sentences this plan makes about the library's
...
the click, exactly one after), and the marked words are text with no `img`, `script` or `iframe` in
the panel. The defect it found was not in the palette at all. The directory **lifts** a real page's
header, which is where the trigger lives, and it was lifting that markup without the script the markup
needs: a control that navigated to the page a reader was already on, and a Cmd+K that did nothing,
looking exactly like a working link and reaching no static check. The generator now lifts
the palette's script tag by name and raises if it is absent rather than shipping a dead control, and
